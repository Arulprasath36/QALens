...
:class:`~qalens.artifacts.config.ArtifactMode` controls storage decisions.
...
4. **Store** (full mode only) — optionally dedupe, optionally compress, write
   to the configured :class:`~qalens.artifacts.storage.ArtifactStore`.
5. **Record** — build :class:`~qalens.artifacts.models.ArtifactRecord` objects
...
from qalens.artifacts.image import compress_image, get_image_dimensions
...
from qalens.artifacts.storage import ArtifactStore, mime_to_ext
    ...
    def process(
        ...
        *,
        ...
        Returns:
            ...
            :meth:`~qalens.db.repository.RunRepository.save_artifacts`.
        """
...
def _decode_ref(ref: ArtifactRef, *, max_bytes: int) -> tuple[bytes, str]:
    """Decode a :class:`~qalens.models.artifact_ref.ArtifactRef` to ``(bytes, mime_type)``.
