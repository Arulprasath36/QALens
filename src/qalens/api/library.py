...
from qalens.parsers.base import DetectionResult
...
if TYPE_CHECKING:
    ...
    from qalens.models.insight import AnalysisSummary, FailureCluster
    from qalens.models.run import TestRun
    from qalens.outputs.digest import DigestData
...
class QALensClient:
    ...
    def detect_report(self, report_path: str | Path) -> DetectionResult:
        ...
        Returns:
            A :class:`~qalens.parsers.base.DetectionResult` with the
            parser key, confidence, and evidence reasons.  Check
            :attr:`~qalens.parsers.base.DetectionResult.matched` to
    ...
    def get_parser_for_path(self, report_path: str | Path) -> BaseParser:
        ...
        Returns:
            The :class:`~qalens.parsers.base.BaseParser` with the highest
    ...
    def extract_report(
        ...
        *,
        ...
        parser, and delegates to its :meth:`~qalens.parsers.base.BaseParser.parse`
        method.
        ...
        Returns:
            A :class:`~qalens.models.run.TestRun` containing all extracted
    ...
    def ingest_report(
        ...
        *,
        ...
        owner_map_path: str | Path | None = None,
        override_owners: bool = False,
        ...
        Args:
            ...
            db_path: Path to the SQLite database file.  When ``None``, the
                default ``~/.qalens/qalens.db`` is used.
            skip_if_exists: Passed through to
                :meth:`~qalens.db.repository.RunRepository.save_run`.
            ...
            owner_map_path: Optional JSON/TOML owner mapping file applied
                before the run is written to the database.
            override_owners: When ``True``, owner mapping rules replace owner
                labels already present in the report.  Defaults to preserving
                report-provided owner labels.
        ...
        """
        from qalens.db.repository import RunRepository
        from qalens.db.schema import default_db_path, get_connection
        ...
        if owner_map_path is not None:
            ...
            mapping = load_owner_mapping(owner_map_path)
            apply_owner_mapping(run, mapping, override_existing=override_owners)
    ...
    def _run_artifact_policy(
        ...
        from qalens.artifacts.config import ArtifactMode
        from qalens.artifacts.models import ArtifactIngestStats
    ...
    def get_repository(
        ...
        """Return an open :class:`~qalens.db.repository.RunRepository`.
        ...
        Returns:
            An initialised :class:`~qalens.db.repository.RunRepository`.
    ...
    def score_flakiness(
        ...
        *,
        ...
        Returns:
            A :class:`~qalens.analyzers.flaky.FlakyResult` for the test.
    ...
    def get_all_flaky(
        ...
        *,
        ...
        Returns:
            List of :class:`~qalens.analyzers.flaky.FlakyResult` sorted by
    ...
    def cluster_report(
        ...
        Returns:
            A list of :class:`~qalens.models.insight.FailureCluster` objects
            ordered by cluster size descending.  Empty list when there are
            no failing tests.

        """
        from qalens.analyzers.clustering import cluster_failures
        from qalens.models.insight import FailureCluster  # noqa: F401 (re-export hint)
    ...
    def ask(
        ...
        *,
        ...
        Raises:
            :exc:`qalens.llm.client.LLMError`: On HTTP or provider errors.
    ...
    def build_digest(
        ...
        *,
        ...
        """Build a :class:`~qalens.outputs.digest.DigestData` from the database.
        ...
        JSON via the functions in :mod:`qalens.outputs.digest`.
        ...
        Returns:
            A :class:`~qalens.outputs.digest.DigestData` instance.

        """
        from qalens.outputs.digest import build_digest
        return build_digest(
            project=project,
            db_path=db_path,
            min_runs=min_runs,
    ...
    def analyze_report(
        ...
        *,
        ...
        1. :class:`~qalens.analyzers.signatures.SignatureEngine` enriches every
           ``FailureInfo`` with ``normalized_message``, ``normalized_stack_trace``,
           and ``failure_signature``.
        2. :func:`~qalens.analyzers.categorizer.categorize_failure` assigns a
           :class:`~qalens.analyzers.categorizer.FailureCategory` to each failure.
        3. One :class:`~qalens.models.insight.Insight` is built per failing test,
           including evidence strings, confidence, and related-test cross-links.
        4. :func:`~qalens.analyzers.clustering.cluster_failures` groups failures by
           shared signature into :class:`~qalens.models.insight.FailureCluster` objects.
        5. :class:`~qalens.models.insight.StatusCounts` and
           :class:`~qalens.models.insight.CategoryCounts` are aggregated.
        ...
        Returns:
            An :class:`~qalens.models.insight.AnalysisSummary` with all insights,
        ...
        from qalens.analyzers.clustering import cluster_failures
        from qalens.analyzers.signatures import SignatureEngine
    ...
    def summarize_report(
        ...
        *,
        ...
        """Render an :class:`~qalens.models.insight.AnalysisSummary` as a string.
...
# ---------------------------------------------------------------------------
# Private rendering helpers (extracted to qalens.api._render)
# ---------------------------------------------------------------------------

from qalens.api._render import (  # noqa: E402, F401
