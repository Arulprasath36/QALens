"""Private rendering helpers for :class:`~qalens.models.insight.AnalysisSummary`.

Extracted from :mod:`qalens.api.library` for cohesion.
"""
...
def _render_analysis_markdown(analysis: "AnalysisSummary") -> str:  # noqa: F821
    ...
    from qalens.models.insight import AnalysisSummary  # noqa: F401 (type annotation)

    sc = analysis.status_counts
    cc = analysis.category_counts

    lines: list[str] = [
        "# QA Lens Analysis Summary",
        "",
        f"**Report:** `{analysis.report_path}`  ",
        f"**Run ID:** `{analysis.run_id}`  ",
        f"**Format:** {analysis.report_format}  ",
        f"**Engine:** qalens v{analysis.analysis_engine_version}",
        "",
        "## Status Overview",
        "",
