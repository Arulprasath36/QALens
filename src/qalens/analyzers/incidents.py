...
Transforms raw :class:`~qalens.db.models.TestCaseRow` failures for a given run
into :class:`~qalens.models.incident.IncidentSummary` objects by:

1. Grouping failures that share the same ``fingerprint`` — identical root-cause
   signal produced by :mod:`qalens.analyzers.fingerprint`.
...
if TYPE_CHECKING:
    ...
    from qalens.db.models import TestCaseRow
...
def assemble_incidents(
    ...
    *,
    ...
    """Build :class:`~qalens.models.incident.IncidentSummary` objects for *run_id*.
