"""Failure signature engine for QA Lens — Phase 4.

The ``SignatureEngine`` is the first analysis step in the QA Lens pipeline.
...
class SignatureEngine:
    ...
    def enrich(self, run: TestRun) -> TestRun:
        ...
        Args:
            run: A ``TestRun`` produced by any QA Lens parser.
    ...
    def enrich_failure_info(self, failure: FailureInfo) -> FailureInfo:
        """Enrich a single :class:`~qalens.models.failure.FailureInfo` in-place.
