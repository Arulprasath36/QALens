"""Analysis route handlers for the QA Lens FastAPI server.
...
def make_analysis_router(db_path: str | Path | None) -> APIRouter:
    ...
    async def failure_groups(
        ...
        run_ids: str | None = Query(None, max_length=4096, description="Comma-separated run IDs to scope counts to a window."),
    ) -> list[dict[str, Any]]:
    ...
    async def risk_predictions(
        project: str | None = Query(None, max_length=200, description="Filter by project name."),
        min_runs: int = Query(2, ge=1, description="Minimum run appearances."),
        tier: str | None = Query(None, max_length=16, description="Filter by tier: CRITICAL, HIGH, MEDIUM, LOW."),
    ) -> list[dict[str, Any]]:
    ...
    async def stability_trends(
        ...
        from qalens.db.repository import RunRepository
        from qalens.db.schema import get_connection, init_db
        from qalens.llm.trend import RunRate, compute_trend
