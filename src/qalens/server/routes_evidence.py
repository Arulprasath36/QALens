...
def make_evidence_router(db_path: str | Path | None) -> APIRouter:
        ...
        try:
            ...
            return {
                ...
                "signals": {
                    ...
                    "recent_decline":  round(signals.recent_decline,  4),
                    "fail_streak":     round(signals.fail_streak,     4),
                    "duration_spike":  round(signals.duration_spike,  4),
    ...
    async def run_evidence(
        run_id: str,
        category: str | None = Query(None, max_length=64, description="Filter by comparison category: newly_failing | recovered | consistently_failing"),
        vs_run_id: str | None = Query(None, max_length=128, description="Older run ID for comparison categories"),
    ) -> dict:
