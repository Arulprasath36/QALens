...
When an :class:`~qalens.llm.answer_plan.AnswerPlan` is supplied, the prompt is
...
Usage::
    ...
    from qalens.llm.answer_plan import detect_answer_intent, build_answer_plan
    from qalens.llm.prompts import build_prompt, build_system_prompt
...
from qalens.security import wrap_untrusted_data
...
from qalens.llm.answer_plan import AnswerIntent  # noqa: E402  (after constants)
...
def _build_scope_disclosure_rules(scope: "DefaultScopeInfo") -> list[str]:  # type: ignore[name-defined]  # noqa: F821
    ...
    These rules tell the LLM to mention the assumed window inline without
    adding footer sections. The workspace and follow-up chips carry evidence
    and refinement affordances now.
    """
    ...
    return [
        ...
        (
            "Do not add 'Scope used', 'Want something more specific?', "
            "or 'You can refine by' sections."
        ),
        (
            "Keep the answer concise; supporting evidence and follow-up options "
            "are shown outside the chat answer."
        ),
    ]
...
def build_system_prompt(answer_plan=None) -> str:
    ...
    Args:
        answer_plan: The :class:`~qalens.llm.answer_plan.AnswerPlan` produced by
            :func:`~qalens.llm.answer_plan.build_answer_plan`.  When ``None`` the
