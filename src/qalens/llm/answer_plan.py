"""Intent-driven answer planning for QA Lens LLM queries.
...
Enums and data models live in :mod:`qalens.llm.answer_types`.
Heuristic keyword cues and detection functions live in
:mod:`qalens.llm.intent_detection`.  This module re-exports them for
...
from qalens.llm.answer_types import (  # noqa: F401
    AnswerIntent,
    AnswerScope,
    AnswerType,
    DefaultScopeInfo,
...
from qalens.llm.intent_detection import (  # noqa: F401
...
class AnswerPlan:
    ...
    Consumed by :func:`~qalens.llm.prompts.build_prompt` and
    :func:`~qalens.llm.prompts.build_system_prompt` to inject structured
...
# detect_secondary_intent) now live in qalens.llm.intent_detection and are
...
def _build_answer_plan_core(
    ...
    if intent == AnswerIntent.RANKING_LIST:
        ...
        _metric_labels: dict[RankingMetric, str] = {
            ...
            RankingMetric.RISK: (
                "QA Lens next-run risk score — combines volatility (flip_score), failure burden "
        ...
        if metric == RankingMetric.RISK:
            list_format_rule = (
                ...
                "  `NUMBER. **`TEST_NAME`** — TIER_EMOJI **TIER** risk · pass rate: PASS_RATE%`\n"
                ...
                "Example: `1. **`testCheckoutCompletes()`** — 🔴 **CRITICAL** risk · pass rate: 49%`. "
            ...
            risk_extra_rules = [
                ...
                "ranked by QA Lens next-run risk score, which combines volatility (flip_score), failure burden, "
                ...
                "    - **Risk tier** — QA Lens's prediction of next-run failure likelihood (CRITICAL > HIGH > MEDIUM > LOW)\n"
                ...
                "then ONE sentence of evidence that explicitly separates the QA Lens risk basis from the pass rate. "
                ...
                "\n"
                "Across the last 10 runs, these are the tests with the highest QA Lens risk score:\n"
                "\n"
                "1. **`testCheckoutCompletes()`** — 🔴 **CRITICAL** risk · pass rate: 32%\n"
                "2. **`testDashboardLoadsInUnder3s()`** — 🟠 **HIGH** risk · pass rate: 43%\n"
                "3. **`testCreditCardPayment()`** — 🟠 **HIGH** risk · pass rate: 90%\n"
                "4. **`testEnableEmailNotifications()`** — 🟡 **MEDIUM** risk · pass rate: 50%\n"
                "5. **`testCancelOrder()`** — 🟡 **MEDIUM** risk · pass rate: 47%\n"
                "\n"
                "There are a total of 10 eligible tests.\n"
                "\n"
                "### 📊 How I ranked them\n"
                "\n"
                "Tests are ranked by QA Lens next-run risk score, which combines volatility "
