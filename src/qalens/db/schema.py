...
Usage::
    ...
    conn = get_connection()          # uses ~/.qalens/qalens.db
...
from qalens.security import validate_sqlite_db_path

# ---------------------------------------------------------------------------
# Default database location
# ---------------------------------------------------------------------------

_DEFAULT_DB_DIR = Path.home() / ".qalens"
_DEFAULT_DB_PATH = _DEFAULT_DB_DIR / "qalens.db"
...
def default_db_path() -> Path:
    ...
    Returns:
        Absolute :class:`~pathlib.Path` to ``~/.qalens/qalens.db``.
    """
...
def get_connection(db_path: str | Path | None = None) -> sqlite3.Connection:
    ...
    Args:
        db_path: Path to the ``.db`` file, or ``":memory:"`` for an
            ...
            default path ``~/.qalens/qalens.db`` is used.
...
CREATE INDEX IF NOT EXISTS idx_fail_tc           ON failures   (tc_id);
