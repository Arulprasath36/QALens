"""Run-data route handlers for the QARA FastAPI server.
...
def make_runs_router(db_path: str | Path | None) -> APIRouter:
    ...
    async def get_attachment(tc_id: str, idx: int) -> Response:
        ...
        return Response(content=resolved.read_bytes(), media_type=mime or "application/octet-stream")
    ...
    async def get_artifact(tc_id: str, artifact_id: int) -> Response:
        ...
        if not resolved.is_file():
            raise HTTPException(status_code=404, detail="Artifact file not found on disk.")
        mime = row["mime_type"] or mimetypes.guess_type(str(resolved))[0] or "application/octet-stream"
    ...
    async def get_run_incidents(run_id: str) -> list[dict[str, Any]]:
        ...
        """
        from qara.analyzers.incidents import assemble_incidents
        from qara.db.repository import RunRepository
        from qara.db.schema import get_connection
