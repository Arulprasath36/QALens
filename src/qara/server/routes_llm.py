...
def _deterministic_context_answer(
    *,
    ...
    if not context_text:
        return (
            "I could not reach the configured LLM, and QARA did not find enough "
    ...
    if mode == "test":
        ...
        if details:
            return (
                "I could not reach the configured LLM, so QARA is returning the "
    ...
    return (
        "I could not reach the configured LLM, so QARA is returning a "
...
def _risk_ranking_result_from_fact_bundle(
    ...
    *,
    ...
    subtitle: str = "Ranked by QARA risk score across the selected run window",
) -> dict:
...
def _finalize_owner_suite_narration(answer: str) -> str:
    ...
    if "narrow" not in lowered and "last 5" not in lowered and "last 10" not in lowered:
        normalized = (
            f"{normalized} If you want, QARA can also narrow this to a particular run window like the last 5 or 10 runs."
        )
...
def _performance_threshold_ms(question: str) -> int | None:
    from qara.utils.text import parse_duration_ms
...
def _load_performance_items(conn, runs: list) -> list[dict]:
    from qara.analyzers.predictor import _duration_trend
...
def _trend_primary_reason(result, kind: str, scope_label: str) -> str:
    ...
    return f"Scored from QARA stability history across {scope_label.lower()}."
...
def _build_stability_trend_result(
    *,
    ...
    return {
        ...
        "subtitle": f"QARA stability analysis across {scope_label.lower()}.",
        "scope": {
            "label": scope_label,
            "runCount": run_count,
            "latestRun": latest_run_label,
...
def _build_owner_suite_result(compare_payload: dict, owner_a: str, owner_b: str) -> dict:
    ...
    return {
        ...
        "subtitle": "Compared by current owner assignment. QARA can narrow this to a specific run window if you want recent health instead.",
        "owners": {
            "ownerA": owner_a,
            "ownerB": owner_b,
...
def make_llm_router(
    ...
    async def ask(
        ...
        from qara.llm.answer_plan import AnswerIntent, RankingMetric, build_answer_plan, detect_answer_intent
        ...
        from qara.server.routes_compare import _build_entity_comparison
        ...
        if root_cause_kind is not None:
            ...
            try:
                ...
                if not available_runs:
                    return AskResponse(
                        answer="I could not find any runs in the available QARA data for this project.",
        ...
        if _is_exception_retrieval_question(body.question):
            ...
            try:
                ...
                if not selected_runs:
                    ...
                    return AskResponse(
                        answer=f"I could not find {scope_text} in the available QARA data.",
                        context_mode="project",
                        sources=[],
                        intent=_answer_intent.value,
                        follow_ups=[],
            ...
            try:
                ...
                if run_number is not None:
                    ...
                    if target_index is None:
                        return AskResponse(
                            answer=f"I could not find Run #{run_number} in the available QARA data.",
                            context_mode="project",
                            sources=[],
                            intent=_answer_intent.value,
                            follow_ups=[],
        ...
        if trend_query_kind is not None:
            ...
            try:
                ...
                if explicit_owner and owner_focus is None:
                    return AskResponse(
                        answer=f'I could not find an owner named "{explicit_owner}" in the available QARA data for this project.',
                        context_mode="project",
                        sources=[],
                        intent=_answer_intent.value,
                        follow_ups=[],
                ...
                if owner_focus is not None:
                    result["subtitle"] = f"QARA stability analysis for {owner_focus} across {scope_label.lower()}."
            ...
            try:
                ...
                if len(run_numbers) >= 2:
                    ...
                    if len(selected_runs) < 2:
                        ...
                        return AskResponse(
                            answer=f"I could not find Run #{missing[0]} in the available QARA data." if missing else "I could not find both runs in the available QARA data.",
                            context_mode="project",
                            sources=[],
                            intent=_answer_intent.value,
                            follow_ups=[],
        ...
        if run_retrieval_kind is not None:
            ...
            try:
                ...
                if run is None:
                    ...
                    return AskResponse(
                        answer=f"I could not find {run_scope} in the available QARA data.",
                        context_mode="project",
                        sources=[],
                        intent=_answer_intent.value,
                        follow_ups=[],
