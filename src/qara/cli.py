...
def ingest(
    ...
    """Parse a report and store it in the local QARA database.
    ...
    db_display = db or (Path.home() / ".qara" / "qara.db")
...
def analyze(
    project: str | None = typer.Option(
        ...
        "--project",
        "-p",
        help="Project name to filter analysis (use value shown by 'qara ingest').",
    ),
    db: Path | None = typer.Option(
        None,
        "--db",
    ...
    Queries the QARA database (populated by 'qara ingest') to produce:
...
def digest(
    ...
    Reads from the QARA database and produces a triage-ready report showing
...
def ask(
    ...
    config: Path | None = typer.Option(
        ...
        "--config",
        "-c",
        help="Path to LLM config TOML. Defaults to ~/.qara/config.toml.",
    ),
    ...
    QARA builds a structured context from your test database and sends it
...
def llm_config(
    ...
    config: Path | None = typer.Option(
        ...
        "--config",
        "-c",
        help="Path to config TOML. Defaults to ~/.qara/config.toml.",
    ),
) -> None:
    """View or update the LLM provider configuration.

    Configuration is stored in [bold]~/.qara/config.toml[/bold].
