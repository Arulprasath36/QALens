"""Signal detection and context orchestration for QARA LLM queries.
...
Usage::
    ...
    from qara.llm.routing import detect_signals, gather_context_for_signals, normalize_query
    ...
    * ``SUMMARY_OVERVIEW``    → :func:`gather_summary_context`
    ...
    if answer_plan is not None:
        ...
        if answer_plan.intent == AnswerIntent.DIAGNOSTIC_ROOT_CAUSE:
            ...
            from qara.llm.answer_plan import _is_flakiness_history_query, _is_flakiness_ranking_query  # type: ignore[attr-defined]
    ...
    from qara.llm.context import gather_risk_context
...
def _scope_from_sources(
    ...
    *,
    ...
    """
    from qara.llm.answer_types import AnswerScope
    ...
    *,
    ...
    sections = [
        ...
        PayloadSection(
            ...
            items=[
                "QARA treats a test as flaky here based on recent pass\u2194fail "
...
def gather_ranking_context(
    *,
    ...
    from qara.analyzers.flaky import FlakyScorer
    from qara.db.schema import get_connection
    from qara.llm.answer_plan import RankingMetric
...
def gather_comparison_context(
    *,
    ...
    if is_trend:
        from qara.llm.trend import RunRate, compute_trend, render_trend_facts
