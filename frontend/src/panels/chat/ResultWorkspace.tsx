...
+ 0.05 × duration_spike`}</pre>
            <p className="text-sm text-slate-600 dark:text-slate-300">
              This ranking is based on the blended QA Lens risk formula, not pass rate alone.
            </p>
          </section>
        </div>
      </aside>
...
function RiskWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Ranked by QA Lens risk score across the selected run window',
    ...
    <div className="flex h-full min-h-[320px] items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white/80 px-8 py-12 text-center shadow-sm dark:border-slate-800 dark:bg-slate-950/80">
      <div className="max-w-md space-y-3">
        ...
        <p className="text-sm leading-7 text-slate-600 dark:text-slate-300">
          Ask a question like “What tests are most likely to fail next?” and QA Lens will keep the conversation on the left while rendering ranked evidence and drill-down analysis here.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-50">{result.title}</h2>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              {result.subtitle ?? 'Current-owner suite overlap and unique suite ownership. Ask QA Lens to narrow this to a specific run window for recent health.'}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
      ...
      <div className="rounded-2xl border border-slate-200 bg-white px-6 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
          This view shows the current suite ownership split only. If you want to compare recent health within these suites, ask QA Lens to narrow the analysis to a time window like the last 5 or 10 runs.
        </p>
      </div>
    </div>
  );
...
function StabilityTrendWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'QA Lens stability analysis.',
      `Scope: ${result.scope.label} • ${result.scope.runCount} run${result.scope.runCount === 1 ? '' : 's'}`,
  ...
  const meaningText = useMemo(() => {
    switch (result.query.kind) {
      ...
      default:
        return 'These tests were selected from QA Lens stability signals across the chosen scope.';
    }
  }, [result]);
        ...
        <div className="border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            {result.tests.length < result.summary.matches
              ? `Showing ${result.tests.length} of ${result.summary.matches} matching tests ranked by QA Lens stability signals.`
              : `${result.summary.matches} matching test${result.summary.matches === 1 ? '' : 's'} ranked by QA Lens stability signals.`}
          </p>
        </div>
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {result.tests.length === 0 ? (
...
function PerformanceTimingWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Execution-time analysis from QA Lens.',
      `Scope: ${result.scope.label} • ${result.scope.runCount} run${result.scope.runCount === 1 ? '' : 's'}`,
      isSlowestQuery
...
function FailureTrendWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Run-by-run failure trend from QA Lens.',
      `Scope: ${result.scope.label}`,
...
function RootCauseInsightWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Evidence-backed root cause analysis from QA Lens.',
      `Scope: ${result.scope.label}`,
  ...
  const meaningText = useMemo(() => {
    if (!result.summary.dominantFamily) {
      return 'QA Lens could not isolate a dominant failure cause from the available evidence, so manual inspection is still required.';
    }
    ...
    if (result.scope.kind === 'cause_mix') {
      return `QA Lens compared UI/test-script style failures against product/backend style failures. The dominant family below shows which side is contributing more of the failure pressure in this scope.`;
    }
      ...
      <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-50">{result.title}</h2>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{result.subtitle ?? 'Evidence-backed triage checklist from recent QA Lens runs.'}</p>
          </div>
          <button
            type="button"
            onClick={handleCopySummary}
...
function RunRetrievalWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Run-level retrieval from QA Lens.',
  ...
  const focusText = useMemo(() => {
    if (result.query.kind === 'status_lookup') {
      return result.tests.length > 0
        ? `QA Lens found ${result.tests.length} matching test${result.tests.length === 1 ? '' : 's'} for this status lookup in ${result.run.label}.`
        : `QA Lens did not find a matching test for this status lookup in ${result.run.label}.`;
    }
...
function ExceptionRetrievalWorkspace({
  ...
  const summaryText = useMemo(() => {
    const lines = [
      result.title,
      result.subtitle ?? 'Exception-based failure retrieval from QA Lens.',
      `Scope: ${result.scope.label}`,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-5 dark:border-blue-500/20 dark:bg-blue-500/10">
        <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-blue-700 dark:text-blue-300">What this shows</h3>
        <p className="mt-3 text-sm leading-7 text-slate-700 dark:text-slate-200">
          QA Lens matched failed tests using exception type and first-line failure message text, so this view catches both exact exception names and close message variants like timeout or element-location errors.
        </p>
      </div>
