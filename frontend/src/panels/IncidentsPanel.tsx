...
function healthSentence(
  severity: ApiIncident['severity'],
  trend: AggregatedIncident['trend'],
  confidence: ApiIncident['confidence'],
): string {
  const impact = {
    critical: 'Wide impact',
    high:     'High impact',
  ...
  const corr = {
    high:   'same root cause',
    medium: 'likely same cause',
    low:    'possibly related failures',
  }[confidence];

  if (trend === 'improving') return `Resolved in latest run — ${corr}`;
  if (trend === 'reducing')  return `Impact reducing — ${corr}`;
  if (trend === 'new')       return `${impact} — newly surfaced, ${corr}`;
  if (trend === 'worsening') return `${impact} — worsening, ${corr}`;
  return `${impact} — persistent, ${corr}`;
}
...
function IncidentCard({
  ...
  return (
    <div
      ...
      className={[
        'qara-accordion transition-[box-shadow] duration-700',
        highlighted ? 'ring-2 ring-warning/50 shadow-[0_0_0_4px_rgb(245_158_11_/_0.12)]' : '',
      ].join(' ')}
    >
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          >
            <span className={`shrink-0 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full
                              text-xs font-semibold border cursor-default ${sev.text} ${sev.bg} ${sev.border}`}>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {scope > 1 && inc.occurrence_count > 1 && (
            <span className="qara-inline-note">
              Seen in {inc.runs_seen_count}/{scope} runs
            </span>
          )}
          <span className="qara-inline-note">
      ...
      {/* ── Body ─────────────────────────────────────────── */}
      {open && (
        <div className="px-4 pb-5 border-t border-border-default pt-4 space-y-4">

          {/* ① Meta line */}
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted">
            {scope > 1 && inc.run_sequences.length > 0 && (
              <>
                <span>First seen <span className="text-primary font-medium">Run #{inc.first_seen_sequence}</span></span>
                <span className="text-faint">·</span>
                <span>Last seen <span className="text-primary font-medium">Run #{inc.last_seen_sequence}</span></span>
                <span className="text-faint">·</span>
              </>
            )}
            <span><span className="text-primary font-medium">{inc.impacted_test_count}</span> tests affected</span>
            <span className="text-faint">·</span>
            <span className="capitalize">
              {inc.root_cause_category === 'unknown' || inc.root_cause_category === 'unclassified'
                ? 'New pattern'
                : inc.root_cause_category.replace(/_/g, ' ')}
            </span>
          </div>

          {/* ② Insight banner — unclassified only */}
          {(inc.root_cause_category === 'unknown' || inc.root_cause_category === 'unclassified') && (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3.5 py-2 border-l-[3px] border-l-amber-400">
              <p className="text-[13px] text-amber-700 font-medium leading-snug">
                New failure pattern detected — not matched to known categories
              </p>
              <p className="mt-0.5 text-xs text-amber-600">
                Requires investigation to establish root cause
              </p>
            </div>
          )}

          {/* ③ Behavior Summary — Tier 1 */}
          {scope > 1 && (() => {
            const cur  = inc.latest_occurrence_count;
            const prev = inc.previous_occurrence_count;
            const seen = inc.runs_seen_count;
            const runRefs = inc.run_sequences.map(seq => `#${seq}`).join(', ');

            const impactLine = cur === 0
              ? <>No impact in the latest run <span className="text-muted">(previous peak {prev} test{prev !== 1 ? 's' : ''})</span></>
              : cur < prev
              ? <>Impact reduced from {prev} <span className="mx-0.5 text-muted">→</span> <span className="font-semibold text-primary">{cur} test{cur !== 1 ? 's' : ''}</span></>
              : cur > prev
              ? <>Impact increased from {prev} <span className="mx-0.5 text-muted">→</span> <span className="font-semibold text-primary">{cur} test{cur !== 1 ? 's' : ''}</span></>
              : <>Impact remains at <span className="font-semibold text-primary">{cur} test{cur !== 1 ? 's' : ''}</span></>;

            const behaviorLabel = cur === 0
              ? 'not present in latest run — likely resolved'
              : seen === scope    ? 'consistently failing'
              : seen <= scope / 2 ? 'intermittent behavior'
              : 'frequently occurring';

            return (
              <div className="border-l-2 border-border-default pl-3.5 py-0.5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted mb-1.5">
                  Behavior summary
                </p>
                <p className="text-sm font-semibold text-primary leading-snug">
                  {healthSentence(inc.severity, inc.trend, inc.confidence)}
                </p>
                <p className="mt-1 text-xs text-muted leading-snug">{impactLine}</p>
                <p className="mt-0.5 text-xs text-muted">
                  Seen in {seen} of {scope} runs
                  {runRefs ? <span className="font-mono"> ({runRefs})</span> : null}
                  {' '}— {behaviorLabel}
                </p>
              </div>
            );
          ...
          {/* ④ Hero root cause — classified incidents only */}
          {inc.root_cause_category !== 'unknown' && inc.root_cause_category !== 'unclassified' && (
          ...
          {/* ⑤ Root cause signal — Tier 2, primary + supporting hierarchy */}
          {inc.evidence.length > 0 && (() => {
            const seen = new Map<string, string>();
            for (const e of inc.evidence) {
              if (/failures span (suites?|areas?)/i.test(e)) continue;
              const key = e.replace(/^\d+\s+/, '').toLowerCase().trim();
              seen.set(key, e);
            }
            const deduped = Array.from(seen.values());
            const [primary, ...supporting] = deduped;
            return (
              <div>
                ...
                <ul className="space-y-1">
                  <li className="flex gap-2 text-sm font-semibold text-primary">
                    <span className="text-muted mt-0.5 shrink-0">•</span>
                    <span><EvidenceText text={primary} /></span>
                  </li>
                  {supporting.length > 0 && supporting.map((e, i) => (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {inc.components.map(c => (
                  <span key={c} className="qara-pill">{c}</span>
                ))}
              </div>
            </div>
          )}
              ...
              <ol className="space-y-1.5 list-none">
                {steps.map((step, i) => (
                  <li key={i} className="flex gap-2.5 text-sm">
                    <span className="shrink-0 w-5 h-5 rounded-full bg-surface-subtle border border-border-default
                                     text-muted text-xs flex items-center justify-center font-medium mt-0.5">
                      {i + 1}
                    </span>
                    <span className="leading-relaxed text-primary">{step}</span>
                  </li>
                ))}
              </ol>
            </div>
          ...
          {/* ⑧ Impacted tests — Tier 3, clearly secondary (collapsible) */}
          {inc.impacted_tests.length > 0 && (
            <div>
              ...
              >
                <svg
                  className={`w-2.5 h-2.5 transition-transform ${testsOpen ? 'rotate-90' : ''}`}
                  viewBox="0 0 12 12" fill="none"
                >
                  <path d="M4 3l3 3-3 3" stroke="currentColor" strokeWidth="1.5"
                        strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                View impacted tests ({inc.impacted_tests.length})
              </button>
              {testsOpen && (
                <ul className="mt-1.5 space-y-0.5 pl-4">
                  {inc.impacted_tests.map((name, i) => (
                    <li key={i} className="flex items-start gap-1.5 text-[11px] text-muted font-mono truncate">
                      <span className="text-faint shrink-0 mt-px">•</span>
                      {name}
                    </li>
                  ))}
                </ul>
          ...
          {inc.representative_stack_trace && (
            <div>
              ...
              <pre className="qara-code-block text-xs text-muted font-mono p-3 overflow-x-auto
                              max-h-48 overflow-y-auto whitespace-pre leading-relaxed">
                {inc.representative_stack_trace}
              </pre>
            </div>
          )}
...
function EnrichedClusterCard({
  ...
  const signatureSnippet = cluster.fingerprint.length > 8
    ...
    : cluster.fingerprint;
  const [primaryEvidence, ...supportingEvidence] = evidence;
