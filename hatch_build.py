...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # type: ignore[override]
        ...
        static_out = Path(__file__).parent / "src" / "qalens" / "server" / "static"
        ...
        if os.getenv("QALENS_SKIP_FRONTEND_BUILD") == "1" and (static_out / "index.html").exists():
            print("QALENS build hook: QALENS_SKIP_FRONTEND_BUILD=1 — skipping npm build.")
            return

        if shutil.which("npm") is None:
            raise RuntimeError(
                "npm not found. Install Node.js ≥18 to build the frontend "
                "(https://nodejs.org), or set QALENS_SKIP_FRONTEND_BUILD=1 "
        ...
        if static_out.exists():
            print(f"QALENS build hook: cleaning previous build at {static_out}")
            shutil.rmtree(static_out)

        print("QALENS build hook: running npm ci...")
        subprocess.run(["npm", "ci"], cwd=frontend, check=True)

        print("QALENS build hook: running npm run build...")
        ...
        print(f"QALENS build hook: frontend built successfully → {static_out}")
